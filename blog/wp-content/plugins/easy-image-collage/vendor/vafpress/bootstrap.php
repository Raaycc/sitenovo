...
// Load Languages       //
//////////////////////////
load_theme_textdomain('easy-image-collage', VP_DIR . '/lang');
	...
	function vp_ajax_wrapper()
	{
		...
		if( VP_Security::instance()->is_function_whitelisted($function) )
		{
			...
			try {
				...
				$result['status']  = true;
				$result['message'] = __("Successful", 'easy-image-collage');
			} catch (Exception $e) {
				$result['data']    = '';
				$result['status']  = false;
		...
		else
		{
			$result['data']    = '';
			$result['status']  = false;
			$result['message'] = __("Unauthorized function", 'easy-image-collage');
		}
